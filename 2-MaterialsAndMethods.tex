...
\begin{equation}
	\label{eq:indlht}
	s_{I}(z) = s_{I}(\theta_{s})	\exp(-\frac{(z - \theta_{s})^2}{2\omega_{s}})
\end{equation}
...
\begin{equation}
	\label{eq:poplht}
	\overline{s_{I}}(\overline{z_{I}}) = s_{I}(\theta_{s}) \sqrt{\frac{\omega_{s}}{\omega_{s}+P_{I}}}	\exp(-\frac{(\overline{z_{I}} - \theta_{s})^2}{2(\omega_{s}+P_{I}})
\end{equation}

Thus at the population level, the mean is:
...

\textbf{Density-dependence!} $s_{0}$ 

\subsection*{Iterations at each timestep}

Using the given parameters, the mean phenotype in each class can be described as \citep{barfield_evolution_2011}
...
