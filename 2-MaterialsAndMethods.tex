...
, with $P_I$ the phenotypic variance in the immature stage. Again, we obtain similar expressions for $\overline{f_1}$ and $\overline{f_2}$.	
...
To induce a trend in the variation of the optima we use the same formula as above but adding a term depending on time $kt$:
...
%
%We supposed an auto-correlated fluctuating environment $\epsilon_{t}$ influencing optima $\theta_{i}$ such as:
%\begin{align}
%	\label{eq:epstheta}
%\left\{
%	\begin{aligned}
