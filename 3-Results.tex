...
\begin{figure}[ht!]
	...
	\caption{\textbf{Effect of density-dependence on phenotypes and populations.} \textbf{No DD} means we used the model without density-dependence, \textbf{DD} means we implemented density-dependence through $s_0$ (see \autoref{eq:ddfunc}). Here, bud-burst date is expressed in julian days (numbered days in the year, 1st of January being 1 in julian days). \textbf{Top:} Phenotype variations in population ($\overline{z_I}, \overline{z_M}$, starting from $z = 116$) with their corresponding genotypic values ($\overline{g_I}, \overline{g_M}$, with $g_i(0) = 116$), and the approximation given by \autoref{eq:zweak}; \textbf{Bottom:} Population, number of immature individuals ($N_I$, red), number of mature individuals ($N_M$, blue). Starting from the density-dependent Stable-Stage Distribution (SSD) in constant environment.}
	\label{fig:dd}
\end{figure}

We used the previously developed model in \citetext{Sandell 2014, master's thesis} and simulated (see~\autoref{fig:dd}) a tree population for 150 years in a constant environment, with and without density-dependence on $s_0$, to assess the effects of a more realistic demography.
